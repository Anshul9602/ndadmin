...
class CandidatesModel extends Model
{
    ...
    public function getrefbyidData($user_id)
    {
    ...
    public function getUserRefData($userId)
    {
        $builder = $this->db->table('users');

        // Select fields from users, agents, hoteliers, and user_profiles
        $builder->select('users.*, agent.*, hoteliers.*, user_profiles.*');

        // Join the agent, hoteliers, and user_profiles tables
        $builder->join('agent', 'agent.user_id = users.id', 'left');
        $builder->join('hoteliers', 'hoteliers.user_id = users.id', 'left');
        $builder->join('user_profiles', 'user_profiles.user_id = users.id', 'left');

        // Filter by the given userId in the users.ref_id field
        $builder->where('users.ref_id', $userId);
    ...
    public function getAllUserData()
    {
        ...
        $builder->where('user_profiles.role', "Job Seeker");
        $query = $builder->get();
        $user = $query->getResult();
        // echo "<pre>";
        // print_r($user);
    ...
    public function getAllHUserData()
    {
        $builder = $this->db->table('users');
        $builder->select('users.*, hoteliers.*');
        $builder->join('hoteliers', 'hoteliers.user_id = users.id');
        $builder->where('hoteliers.role', "Employers");
        $query = $builder->get();
        $user = $query->getResult();
        // echo "<pre>";
        // print_r($user);
    ...
    public function getAllAUserData()
    {
        $builder = $this->db->table('users');
        $builder->select('users.*, agent.*');
        $builder->join('agent', 'agent.user_id = users.id');
        $builder->where('agent.role', "Agent");
        $query = $builder->get();
        $user = $query->getResult();
        // echo "<pre>";
        // print_r($user);
    ...
    public function update_status_d($id)
    {
        ...
        if ($result) {
            ...
            $sql = "UPDATE `users` SET `status`='$new_status',`updated_at`='$date1' WHERE `id` = $user_id";
            $post = $this->db->query($sql);
